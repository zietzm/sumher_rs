...
double cg_solve(double *mat, int length, double *mat2, double *mat3, int ncol,
  ...
  // mat2 stores the xks - at start contains initial guess, at end contains
  // solution mat3 starts as rhs, will be used to store rs mat4 stores the pks
  ...
  for (j = 0; j < ncol; j++) {
    rktrk2[j] = 0;
    for (i = 0; i < length; i++) {
      rktrk2[j] += pow(mat3[i + j * length], 2);
    }
  }
  ...
  alpha = -1.0;
  beta = 1.0;
  dgemm_("N", "N", &length, &ncol, &length, &alpha, mat, &length, mat2, &length,
         &beta, mat3, &length);

  // p0=r0 - saved in mat4
  for (j = 0; j < ncol; j++) {
    for (i = 0; i < length; i++) {
      mat4[i + j * length] = mat3[i + j * length];
    }
  }
  ...
  count2 = 0;
  for (j = 0; j < ncol; j++) {
    count2 += (fabs(rktrk[j] / rktrk2[j]) < tol);
  }
  for (j = 0; j < ncol; j++) {
    if (j == 0) {
      diff = fabs(rktrk[j] / rktrk2[j]);
    }
    if (fabs(rktrk[j] / rktrk2[j]) > diff) {
      diff = fabs(rktrk[j] / rktrk2[j]);
    }
  }
  ...
  for (count = 0; count < length; count++) {
    ...
    alpha = 1.0;
    beta = 0.0;
    dgemm_("N", "N", &length, &ncol, &length, &alpha, mat, &length, mat4,
           &length, &beta, Apk, &length);

    // alphaks = rkTrk / pkT Apk
    for (j = 0; j < ncol; j++) {
      value = 0;
      for (i = 0; i < length; i++) {
        value += mat4[i + j * length] * Apk[i + j * length];
      }
    ...
    for (j = 0; j < ncol; j++) {
      for (i = 0; i < length; i++) {
        mat2[i + j * length] += alphak[j] * mat4[i + j * length];
      }
    }
    ...
    for (j = 0; j < ncol; j++) {
      for (i = 0; i < length; i++) {
        mat3[i + j * length] -= alphak[j] * Apk[i + j * length];
      }
    }
    ...
    for (j = 0; j < ncol; j++) {
      for (i = 0; i < length; i++) {
        mat4[i + j * length] =
            mat3[i + j * length] + betak[j] * mat4[i + j * length];
      }
    }
  } // end of while loop
...
int eigen_invert(double *mat, int length, double *mat2, int ncol, double *mat3,
  ...
  if (info != 0) {
    printf(
        "Error, eigen decomp failed; please tell Doug (info %d, length %d)\n\n",
        info, length);
    exit(1);
  }
  free(work);
  ...
  if (ncol == 0) // solve XXT = mat, return in mat - X = UE^.5
  {
    for (j = 0; j < length; j++) {
      if (mat2[j] > 0.000001) {
        value = pow(mat2[j], .5);
        for (i = 0; i < length; i++) {
          mat[(size_t)j * length + i] = mat[(size_t)j * length + i] * value;
        }
      } else {
        for (i = 0; i < length; i++) {
          mat[(size_t)j * length + i] = 0;
        }
      }
    }
  }
  ...
  if (ncol == -1) // solve X mat = I - therefore mat becomes UE^-1UT
  {
    ...
    for (j = 0; j < length; j++) {
      if (fabs(mat2[j]) > 0.000001) {
        ...
        for (i = 0; i < length; i++) {
          mat3[(size_t)j * length + i] = mat[(size_t)j * length + i] / value;
        }
      } else {
        for (i = 0; i < length; i++) {
          mat3[(size_t)j * length + i] = 0;
        }
      }
    }

    // get number of positive eigenvalues (eigenvalues ranked in ascending
    // order)
    count2 = length - count;
    ...
    if (count2 > 0) // deal with last count2 vectors (positive eigenvalues)
    {
      alpha = 1.0;
      beta = 0.0;
      dgemm_("N", "T", &length, &length, &count2, &alpha, mat3 + count * length,
             &length, mat3 + count * length, &length, &beta, mat, &length);
    }
    ...
    if (count > 0) // deal with first count vectors (negative eigenvalues)
    {
      ...
      dgemm_("N", "T", &length, &length, &count, &alpha, mat3, &length, mat3,
             &length, &beta, mat, &length);
    }
  } // end of ncol=-1
  ...
  if (ncol > 0) // solve mat X = mat3 - X = UE^-1UT mat3
  {
    ...
    alpha = 1.0;
    beta = 0.0;
    dgemm_("T", "N", &length, &ncol, &length, &alpha, mat, &length, mat3,
           &length, &beta, mat4, &length);
    for (i = 0; i < length; i++) {
      for (j = 0; j < ncol; j++) {
        if (fabs(mat2[i]) >= 0.000001) {
          mat4[(size_t)j * length + i] = mat4[(size_t)j * length + i] / mat2[i];
        } else {
          mat4[(size_t)j * length + i] = 0;
        }
      }
    }
    dgemm_("N", "N", &length, &ncol, &length, &alpha, mat, &length, mat4,
           &length, &beta, mat3, &length);
    free(mat4);
  }
...
void eigen_strip(double *mat, int start, int end, int total, double strip) {
  ...
  for (i = 0; i < length; i++) {
    for (j = 0; j < length; j++) {
      U[(size_t)j * length + i] =
          mat[(size_t)(start + j) * total + (start + i)];
    }
  }
  ...
  for (j = 0; j < length; j++) {
    value = pow(E[j], .5);
    for (i = 0; i < length; i++) {
      U[(size_t)j * length + i] *= value;
    }
  }
  alpha = 1.0;
  beta = 0.0;
  dgemm_("N", "T", &length, &length, &length, &alpha, U, &length, U, &length,
         &beta, mat + start + start * total, &total);
...
double ldlt_invert(double *mat, int length, int ncol, double *mat2, int *info,
  ...
  if (*info != 0) {
    printf(
        "Error, LDLT priming failed; please tell Doug (info %d, length %d)\n\n",
        *info, length);
    exit(1);
  }
  lwork = (int)wkopt;
  ...
  if (*info == 0) // decomposition worked
  {
    ...
    det = 0;
    for (i = 0; i < length; i++) {
      if (fabs(mat[(size_t)i * length + i]) >= 0.000001) {
        det += log(fabs(mat[(size_t)i * length + i]));
      } else {
        det += log(0.000001);
      }
    }
    ...
    if (ncol == -1) // put inverse into mat
    {
      ...
      if (*info != 0) {
        printf("Error LDLT invert failed, please tell Doug (info %d, length "
               "%d)\n\n",
               *info, length);
        exit(1);
      }
      for (i = 0; i < length; i++) {
        for (j = 0; j < i; j++) {
          mat[(size_t)j * length + i] = mat[(size_t)i * length + j];
        }
      }
      free(work);
    }
    ...
    if (ncol > 0) // solve
    {
      ...
      if (*info != 0) {
        printf("Error LDLT solve failed, please tell Doug (info %d, length "
               "%d)\n\n",
               *info, length);
        exit(1);
      }
...
double cholesky_invert(double *mat, int length, int ncol, double *mat2,
  ...
  if (*info == 0) // decomposition worked
  {
    ...
    for (i = 0; i < length; i++) {
      if (fabs(mat[(size_t)i * length + i]) >= 0.000001) {
        det += 2 * log(fabs(mat[(size_t)i * length + i]));
      } else {
        det += 2 * log(0.000001);
      }
    }
    ...
    if (ncol == -1) // put inverse into mat
    {
      ...
      if (*info != 0) {
        printf("Error Cholesky invert failed, please tell Doug (info %d, "
               "length %d)\n\n",
               *info, length);
        exit(1);
      }
    ...
    if (ncol > 0) // solve
    {
      ...
      if (*info != 0) {
        printf("Error Cholesky solve failed, please tell Doug (info %d, length "
               "%d)\n\n",
               *info, length);
        exit(1);
      }
  ...
  } else // decomposition failed
  {
    if (type == 1) {
      printf(
          "Warning, Cholesky failed (info %d, length %d); please tell Doug\n",
          *info, length);
    }
    det = 0;
  }
